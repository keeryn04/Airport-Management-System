    ...
    for (size_t j = 0; j < a.getFlightVector().size(); j++) {
        for (int i = 0; i < a.getFlight(j).get_numPass(); i++) { // Check through each passenger instance
            if (a.getFlight(j).get_Passenger(i).get_id() == pass_id) { // Find passenger with that pass_id and delete them
                ...
                Our_Seat removedSeat = *(a.getFlight(j).get_Passenger(i).get_Seat()); //Saves seat for use later
                a.getFlight(j).get_Seat_in_map(removedSeat.get_row(), removedSeat.get_col()).set_status('O'); //Removes seat from seatmap
                a.getFlight(j).removePassenger(i); //Removes passenger
