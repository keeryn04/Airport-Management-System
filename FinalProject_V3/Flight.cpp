...
void Our_Flight::set_Passenger(const Our_Passenger& passengerInfo, size_t passNum) {
    if (passNum >= 0) {
        if (passNum >= Passengers.size()) {
            Passengers.resize(passNum + 1); //Extra check, resizes Passenger vector if too small (Should do this in default constructor)
        }
        Passengers[passNum] = passengerInfo; //Checks if valid passenger, then creates new passenger instance with copy of that info at passenger index
    } else {
